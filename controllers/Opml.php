...
class Opml extends BaseController {
    ...
    */
        ...
        try {
            ...
            // On success bring them back to their subscription list
                ...
                $this->msg = "Success! " . $amount . " feed" . ($amount!=1?"s have":" has") . " been imported.<br />" .
                             "You might want to <a rel='nofollow' href='update'>update now</a> or <a rel='nofollow' href='./'>view your feeds</a>.";
    ...
    private function processGroup($xml, $tags = Array()) {
        ...
        // parse outline items from the default and shomyo namespaces
        foreach ($xml->xpath("outline|shomyo:outline") as $outline) {
            if (count($outline->children()) + count($outline->children('shomyo', true)) > 0) {
    ...
    private function addSubscription($xml, $tags) {
        ...
        $nsattrs = $xml->attributes('shomyo', true);
        ...
        if ($nsattrs->spout || $nsattrs->params) {
            if (!($nsattrs->spout && $nsattrs->params)) {
                ...
                \F3::get('logger')->log('                                     missing attribute ' .
                                        ($nsattrs->spout ? '"shomyo:params"' : '"shomyo:spout"'), \DEBUG);
                return $title;
            }
    ...
    private function writeSource($source) {
        ...
        $this->writer->writeAttributeNS('shomyo', 'spout', null, $source['spout']);
    ...
    public function export() {
        ...
        $this->writer->writeAttribute('generatedBy', 'shomyo-' . \F3::get('version'));
        ...
        foreach ($sources['tagged'] as $tag => $children) {
            ...
            $this->writer->writeAttributeNS('shomyo', 'color', null, $tagColors[$tag]);
